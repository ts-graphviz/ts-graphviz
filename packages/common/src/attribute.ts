 ...
 *
 * @typeParam T - The {@link AttributeKey} to be mapped to a value.
 * @public
 */
...
export namespace Attribute {
  ...
  export interface $keys {
     ...
     * Note that the edge attribute {@link (Attribute:namespace).$keys.len} can be used to override this value for adjacent nodes.
     *
     ...
     *
     * Note that, for edges, the attributes {@link (Attribute:namespace).$keys.headURL}, {@link (Attribute:namespace).$keys.tailURL}, {@link (Attribute:namespace).$keys.labelURL} and {@link (Attribute:namespace).$keys.edgeURL} allow control of various parts of an edge.
     ...
     * During rendering, the canvas is first filled as described in the {@link (Attribute:namespace).$keys.bgcolor} attribute.
     ...
     * This will only appear if the {@link (Attribute:namespace).$keys.dir} attribute is "forward" or "both".
     *
     ...
     * This will only appear if the {@link (Attribute:namespace).$keys.dir} attribute is "back" or "both".
     *
     * See the {@link https://graphviz.org/doc/info/attrs.html#undir_note | limitation}.
     *
     ...
     * If a cluster has a filled {@link (Attribute:namespace).$keys.style}, the cluster's {@link (Attribute:namespace).$keys.fillcolor} will overlay the background color.
     *
    ...
    /**
     * If true, allow edges between clusters. (See {@link (Attribute:namespace).$keys.lhead} and {@link (Attribute:namespace).$keys.ltail} below.)
     *
     ...
     * If set too small, connected components may overlap. Only applicable if {@link (Attribute:namespace).$keys.pack}=false.
     *
     ...
     * The actual style of the arrowhead can be specified using the {@link (Attribute:namespace).$keys.arrowhead} and {@link (Attribute:namespace).$keys.arrowtail} attributes.
    ...
    /**
     * Only valid when {@link (Attribute:namespace).$keys.mode}="ipsep".
     ...
     * If "hier", generates level constraints similar to those used with {@link (Attribute:namespace).$keys.mode}="hier".
    ...
    /**
     * Distortion factor for {@link (Attribute:namespace).$keys.shape}=polygon.
     ...
     * This value overrides any {@link (Attribute:namespace).$keys.URL} defined for the edge.
     * Also, this value is used near the head or tail node unless overridden by a {@link (Attribute:namespace).$keys.headURL} or {@link (Attribute:namespace).$keys.tailURL} value, respectively.
     * See {@link https://graphviz.org/doc/info/attrs.html#undir_note | limitation}.
     *
    ...
    /**
     * Synonym for {@link (Attribute:namespace).$keys.edgeURL}.
     *
    ...
    /**
     * If the edge has a {@link (Attribute:namespace).$keys.URL} or {@link (Attribute:namespace).$keys.edgeURL} attribute, this attribute determines which window of the browser is used for the URL attached to the non-label part of the edge.
     * Setting it to "_graphviz" will open a new window if it doesn't already exist, or reuse it if it does.
     * If undefined, the value of the {@link (Attribute:namespace).$keys.target} is used.
     *
     ...
     * This is used only if the edge has a {@link (Attribute:namespace).$keys.URL} or {@link (Attribute:namespace).$keys.edgeURL} attribute.
     *
     ...
     * The interpretation is the same as given for {@link (Attribute:namespace).$keys.sep}.
     * This should normally be strictly less than {@link (Attribute:namespace).$keys.sep}.
     *
    ...
    /**
     * Color used to fill the background of a node or cluster assuming {@link (Attribute:namespace).$keys.style}=filled, or a filled arrowhead.
     * If fillcolor is not defined, {@link (Attribute:namespace).$keys.color} is used. (For clusters, if color is not defined, {@link (Attribute:namespace).$keys.bgcolor} is used.)
     * If this is not defined, the default is used, except for {@link (Attribute:namespace).$keys.shape}=point or when the output format is MIF, which use black by default.
     *
     ...
     * See also the {@link (Attribute:namespace).$keys.gradientangle} attribute for setting the gradient angle.
     *
     ...
     * if any, with a {@link (Attribute:namespace).$keys.margin} specified by the margin attribute.
     * The width and height must also be at least as large as the sizes specified by the {@link (Attribute:namespace).$keys.width} and {@link (Attribute:namespace).$keys.height} attributes,
     * which specify the minimum values for these parameters.
     *
     * If `true`, the node size is specified by the values of the {@link (Attribute:namespace).$keys.width} and {@link (Attribute:namespace).$keys.height} attributes only and is not expanded to contain the text label.
     * There will be a warning if the label (with margin) cannot fit within these limits.
     *
     * If the {@link (Attribute:namespace).$keys.fixedsize} attribute is set to shape, the {@link (Attribute:namespace).$keys.width} and {@link (Attribute:namespace).$keys.height} attributes also determine the size of the node shape,
     ...
     * If you specify fontname=schlbk, the tool will look for a file named schlbk.ttf or schlbk.pfa or schlbk.pfb in one of the directories specified by the {@link (Attribute:namespace).$keys.fontpath} attribute.
    ...
    /**
     * If true, all {@link (Attribute:namespace).$keys.xlabel} attributes are placed,
     ...
     * Also, this value is used near the head node, overriding any {@link (Attribute:namespace).$keys.URL} value.
     * See {@link https://graphviz.org/doc/info/attrs.html#undir_note | limitation}.
     *
    ...
    /**
     * Synonym for {@link (Attribute:namespace).$keys.headURL}.
     *
    ...
    /**
     * If the edge has a {@link (Attribute:namespace).$keys.headURL}, this attribute determines which window of the browser is used for the URL.
     * Setting it to "_graphviz" will open a new window if it doesn't already exist, or reuse it if it does.
     * If undefined, the value of the {@link (Attribute:namespace).$keys.target} is used.
     *
     ...
     * This is used only if the edge has a {@link (Attribute:namespace).$keys.headURL} attribute.
     *
     ...
     * If {@link (Attribute:namespace).$keys.fixedsize} is true, this will be the final height of the node.
    ...
    /**
     * Synonym for {@link (Attribute:namespace).$keys.URL}.
     *
     ...
     *
     * Unlike with the {@link (Attribute:namespace).$keys.shapefile} attribute, the image is treated as node content rather than the entire node. In particular, an image can be contained in a node of any shape, not just a rectangle.
     *
    ...
    /**
     * Specifies a list of directories in which to look for image files as specified by the {@link (Attribute:namespace).$keys.image} attribute or using the IMG element in {@link https://graphviz.org/doc/info/shapes.html | HTML-like labels}.
     ...
     * In general, the image is given its natural size, (cf. {@link (Attribute:namespace).$keys.dpi}), and the node size is made large enough to contain its image, its label, its margin, and its peripheries.
     * Its width and height will also be at least as large as its minimum {@link (Attribute:namespace).$keys.width} and {@link (Attribute:namespace).$keys.height}.
     ...
     * If a node's {@link (Attribute:namespace).$keys.shape} is record, then the label can have a {@link https://graphviz.org/doc/info/shapes.html | special format} which describes the record layout.
     *
    ...
    /**
     * This, along with {@link (Attribute:namespace).$keys.labeldistance}, determine where the headlabel (taillabel) are placed with respect to the head (tail) in polar coordinates.
     ...
     * See {@link (Attribute:namespace).$keys.labelangle} for more details.
     *
    ...
    /**
     * Synonym for {@link  (Attribute:namespace).$keys.labelURL}.
     *
     ...
     * Note that a subgraph inherits attributes from its parent.
     * Thus, if the root graph sets {@link (Attribute:namespace).$keys.labelloc} to "b", the subgraph inherits this value.
     *
    ...
    /**
     * If the edge has a {@link (Attribute:namespace).$keys.URL} or {@link (Attribute:namespace).$keys.labelURL} attribute, this attribute determines which window of the browser is used for the URL attached to the label.
     * Setting it to "_graphviz" will open a new window if it doesn't already exist, or reuse it if it does.
     * If undefined, the value of the {@link (Attribute:namespace).$keys.target} is used.
     *
     ...
     * This is used only if the edge has a {@link (Attribute:namespace).$keys.URL} or {@link (Attribute:namespace).$keys.labelURL} attribute.
     *
     ...
     * Synonymous with {@link (Attribute:namespace).$keys.rotate | rotate=90} or {@link (Attribute:namespace).$keys.orientation | orientation=landscape}.
     *
    ...
    /**
     * Specifies the separator characters used to split the {@link (Attribute:namespace).$keys.layers} attribute into a list of layer names.
     *
    ...
    /**
     * Specifies strictness of level constraints in neato when {@link (Attribute:namespace).$keys.mode}="ipsep" or "hier".
     ...
     * When {@link (Attribute:namespace).$keys.compound} is true, if **lhead** is defined and is the name of a cluster containing the real head,
     ...
     * When {@link (Attribute:namespace).$keys.compound} is true, if **ltail** is defined and is the name of a cluster containing the real tail, the edge is clipped to the boundary of the cluster.
     * See the {@link https://graphviz.org/doc/info/attrs.html#undir_note | limitation}.
     *
     ...
     * To actually extend the background of a drawing, see the {@link (Attribute:namespace).$keys.pad} attribute.
     *
     ...
     * which allows the graph to specify minimum vertical and horizontal distances between nodes. (See the {@link (Attribute:namespace).$keys.sep} attribute.)
     *
     ...
     * By default, it uses the length of the shortest path, where the length of each edge is given by its {@link (Attribute:namespace).$keys.len} attribute.
     ...
     * In this case, the {@link (Attribute:namespace).$keys.len} of an edge is used as the ideal distance between its vertices.
    ...
    /**
     * If Graphviz is built with MOSEK defined, mode=ipsep and mosek=true, the Mosek software ([www.mosek.com](https://www.mosek.com)) is use to solve the ipsep constraints.
     *
     ...
     * Thus, in the label of a polygonal node, a left-justified line will align with the left side of the node (shifted by the prescribed {@link (Attribute:namespace).$keys.margin}).
    ...
    /**
     * If "[lL]*", set graph orientation to landscape Used only if {@link (Attribute:namespace).$keys.rotate} is not defined.
     *
     ...
     * Nodes are first enlarged using the {@link (Attribute:namespace).$keys.sep} attribute.
     ...
     * This technique starts with a small scaling up, controlled by the {@link (Attribute:namespace).$keys.overlap_scaling} attribute, which can remove a significant portion of the overlap.
     ...
     *
     * If the layout is done by neato with {@link (Attribute:namespace).$keys.mode}="ipsep", then one can use overlap=ipsep.
     ...
     * If pack has an integral value, this is used as the size, in {@link https://graphviz.org/doc/info/attrs.html#points | points}, of a margin around each part; otherwise, a default margin of 8 is used.
     * If pack is interpreted as false, the entire graph is laid out together.
     * The granularity and method of packing is influenced by the {@link (Attribute:namespace).$keys.packmode} attribute.
     *
     ...
     * Note that defining {@link (Attribute:namespace).$keys.packmode} will automatically turn on packing as though one had set `pack=true`.
     *
     ...
     * The pages are then produced one at a time, in {@link (Attribute:namespace).$keys.pagedir} order.
     *
     ...
     * Or use the {@link (Attribute:namespace).$keys.viewport} to generate multiple files.
     *
     ...
     * If **pencolor** is not defined, {@link (Attribute:namespace).$keys.color} is used.
     * If this is not defined, {@link (Attribute:namespace).$keys.bgcolor} is used.
     ...
     *
     * Previous to 31 January 2008, the effect of penwidth=W was achieved by including setlinewidth(W) as part of a {@link (Attribute:namespace).$keys.style} specification.
     ...
     *
     * After 27 Feb 2014, this translation can be avoided in neato by setting the {@link (Attribute:namespace).$keys.notranslate} to TRUE.
     * However, if the graph specifies {@link (Attribute:namespace).$keys.overlap | node overlap removal} or a change in {@link (Attribute:namespace).$keys.ratio | aspect ratio}, node coordinates may still change.
     *
     ...
     * Note that this is adjusted before the {@link (Attribute:namespace).$keys.size} attribute constraints are enforced.
     ...
     *
     * If **ratio** = "fill" and the {@link (Attribute:namespace).$keys.size} attribute is set, node positions are scaled, separately in both x and y, so that the final drawing exactly fills the specified size.
     * If both {@link (Attribute:namespace).$keys.size} values exceed the width and height of the drawing, then both coordinate values of each node are scaled up accordingly.
     * However, if either size dimension is smaller than the corresponding dimension in the drawing, one dimension is scaled up so that the final drawing has the same aspect ratio as specified by {@link (Attribute:namespace).$keys.size}.
     * Then, when rendered, the layout will be scaled down uniformly in both dimensions to fit the given {@link (Attribute:namespace).$keys.size}, which may cause nodes and text to shrink as well.
     ...
     *
     * If **** = "compress" and the {@link (Attribute:namespace).$keys.size} attribute is set, dot attempts to compress the initial layout to fit in the given size.
     * This achieves a tighter packing of nodes but reduces the balance and symmetry.
     * This feature only works in dot.
     *
     * If **** = "expand", the {@link (Attribute:namespace).$keys.size} attribute is set, and both the width and the height of the graph are less than the value in {@link (Attribute:namespace).$keys.size},
     * node positions are scaled uniformly until at least one dimension fits {@link (Attribute:namespace).$keys.size} exactly.
     * Note that this is distinct from using {@link (Attribute:namespace).$keys.size} as the desired size, as here the drawing is expanded before edges are generated and all node and text sizes remain unchanged.
     *
     * If **** = "auto", the {@link (Attribute:namespace).$keys.page} attribute is set and the graph cannot be drawn on a single page, then {@link (Attribute:namespace).$keys.size} is set to an `"ideal"` value.
    ...
    /**
     * This is a synonym for the {@link (Attribute:namespace).$keys.dpi} attribute.
     *
    ...
    /**
     * If the input graph defines the {@link (Attribute:namespace).$keys.vertices} attribute, and output is dot or xdot, this gives the number of points used for a node whose shape is a circle or ellipse.
     ...
     *
     * If unset but {@link (Attribute:namespace).$keys.esep} is defined, the sep values will be set to the esep values divided by 0.8. If esep is unset, the default value is used.
     *
     ...
     * The {@link (Attribute:namespace).$keys.shape} of the node is set to box.
     ...
     * @graphvizDefault ""
     * @graphvizUsedBy N
     * @deprecated Graphviz deprecates the use of this attribute.
     */
    shapefile: 'shapefile';
    ...
    /**
     * Number of sides if {@link (Attribute:namespace).$keys.shape}=polygon.
     *
     ...
     *
     * Note that there is some interaction between the **size** and {@link (Attribute:namespace).$keys.ratio} attributes.
     *
    ...
    /**
     * Skew factor for {@link (Attribute:namespace).$keys.shape}=polygon. Positive values skew top of polygon to right; negative to left.
     *
    ...
    /**
     * If {@link (Attribute:namespace).$keys.packmode} indicates an array packing, this attribute specifies an insertion order among the components,
     ...
     * Note that for these latter layouts, if splines="true", this requires non-overlapping nodes (cf. {@link (Attribute:namespace).$keys.overlap}). If fdp is used for layout and splines="compound",
     ...
     * Also, this value is used near the tail node, overriding any {@link (Attribute:namespace).$keys.URL} value.
     * See {@link https://graphviz.org/doc/info/attrs.html#undir_note | limitation}.
     *
    ...
    /**
     * Synonym for {@link (Attribute:namespace).$keys.tailURL}.
     *
    ...
    /**
     * If the edge has a {@link (Attribute:namespace).$keys.tailURL}, this attribute determines which window of the browser is used for the URL.
     * Setting it to "_graphviz" will open a new window if it doesn't already exist, or reuse it if it does.
     * If undefined, the value of the {@link (Attribute:namespace).$keys.target} is used.
     *
     ...
     * This is used only if the edge has a {@link (Attribute:namespace).$keys.tailURL} attribute.
     *
     ...
     * If unset, Graphviz will use the object's {@link (Attribute:namespace).$keys.label} if defined.
     ...
     * If the attribute is unset, truecolor is not used unless there is a {@link (Attribute:namespace).$keys.shapefile} property for some node in the graph.
     ...
     * If the node is an ellipse or circle, the {@link (Attribute:namespace).$keys.samplepoints} attribute affects the output.
     *
     ...
     * Note that this attribute supersedes any {@link (Attribute:namespace).$keys.size} attribute.
     ...
     * If {@link (Attribute:namespace).$keys.fixedsize} is true, this will be the final width of the node.
     ...
     * For other layouts, the xlabel attribute can be viewed as a synonym for the {@link (Attribute:namespace).$keys.label} attribute.
     *
     ...
     * To force placing all of them, use the {@link (Attribute:namespace).$keys.forcelabels} attribute.
     *
    ...
    /**
     * Deprecated:Use {@link (Attribute:namespace).$keys.pos} attribute, along with {@link (Attribute:namespace).$keys.dimen} and/or {@link (Attribute:namespace).$keys.dim} to specify dimensions.
     *
     * Provides z coordinate value for 3D layouts and displays.
     * If the graph has {@link (Attribute:namespace).$keys.dim} set to 3 (or more), neato will use a node's **z** value for the z coordinate of its initial position if its {@link (Attribute:namespace).$keys.pos} attribute is also defined.
     *
