...
export function createProgram(): Command {
  ...
  program
    ...
    .argument(
      '<packages...>',
      'Test package patterns (glob patterns supported)',
    )
    .option(
      '-c, --config <file>',
...
export async function loadConfigFromFile(
  configPath: string,
): Promise<Partial<E2ERunnerConfig>> {
  try {
 ...
 */
export async function expandTestPackageGlobs(
  patterns: string[],
): Promise<TestPackage[]> {
  ...
  if (testPackages.length === 0) {
    throw new Error(
      `No valid test packages found for patterns: ${patterns.join(', ')}`,
    );
  }
...
function inferPackageType(packageJson: any): string {
  const hasTypeScript = ['tsx', 'typescript', '@types/node'].some(
    (dep) =>
